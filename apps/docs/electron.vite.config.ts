...
const localAlias = {
  '@trivoffice/docx-engine': resolve(__dirname, '../../packages/docx-engine/src/index.ts'),
}
...
export default defineConfig({
  ...
  // @trivoffice/* deps ship as raw TS source with extensionless imports, so they
  ...
  renderer: {
    ...
    resolve: { alias: localAlias },
    server: {
      // Overridable so multiple trivoffice dev instances can coexist (default 5173).
