 ...
 */
import type { RenderSlide } from '@trivoffice/pptx-render'
import type { SlideComment, SectionInfo } from '@trivoffice/pptx-engine'
import type {
  AiSettings,
  AiStreamChunk,
  AiStreamRequest,
...
export type { SlideComment, SectionInfo } from '@trivoffice/pptx-engine'

// Canonical definitions of AI-related types live in @trivoffice/ai-provider / @trivoffice/agent-core (shared with docs)
export type {
  AiProviderConfig,
  AiProviderId,
  AiProviderMeta,
...
export type { AgentToolCall, AgentToolDef } from '@trivoffice/agent-core'
...
export interface SlidesApi {
  ...
  htmlToPptx: (
    ...
    | { error: string }
  >
  /** Whether cloud single-page generation (gsk slide_generate) is available (TRIVOFFICE_CLOUD_SLIDE=1 + gsk login) */
  ...
  /** Trivena Cloud account status (gsk login state); with withEmail also fetches the email (needs a network request, slower) */
  aiGskStatus: (withEmail?: boolean) => Promise<GenSparkAccountStatus>
  /** Open the browser to log into Trivena Cloud (fire-and-forget; aiGskStatus turns logged-in once done) */
  ...
  /** gsk (Trivena Cloud) AI image generation/editing, returns the image URL (error prompts login when logged out) */
  generateImage: (op: {
    prompt: string
    model?: string
  ...
  /** gsk (Trivena Cloud) media analysis: image/audio/video content understanding, returns analysis text */
  analyzeMedia: (op: {
    mediaUrls: string[]
    requirements: string
