...
export function registerSheetsAiIpc(): void {
  ...
  ipcMain.handle(IPC_CHANNELS.aiGetSettings, (event): AiSettings => {
    ...
    // AI features all go through Trivena Cloud (gsk login); legacy settings that chose
    // another provider are reset
    settings.provider = 'genspark'
    return settings
  })

  // Trivena Cloud account (gsk login state): the auth source for AI features; the
