...
import { gskApiKey, gskListPastProjects, hasGskAuth } from '@trivoffice/ai-search'
...
export function cloudStoreOwner(): string {
  const key = gskApiKey()
  if (!key) return ''
  return createHash('sha256').update(`trivoffice-cloud-store:${key}`).digest('hex').slice(0, 16)
}
