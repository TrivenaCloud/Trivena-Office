...
import { createI18n, getUiLang, htmlLang } from '@trivoffice/i18n'
 ...
 * itself is injected at build time via the TRIVOFFICE_UPDATE_URL env var and
 ...
 *
 * Dev preview: TRIVOFFICE_FAKE_UPDATE=<version> in an unpacked run opens the
...
const DOWNLOAD_PAGE_URL = 'https://github.com/TrivenaCloud/Trivena-Office/releases/latest'
...
export function initAutoUpdater(
  ...
  if (!app.isPackaged && process.env.TRIVOFFICE_FAKE_UPDATE) {
    initFakeUpdate(getWindow, process.env.TRIVOFFICE_FAKE_UPDATE)
    return
  }
