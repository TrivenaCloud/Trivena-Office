...
import { createI18n, getUiLang } from '@trivoffice/i18n'
import { gskGenerateImage, hasGskAuth } from '@trivoffice/ai-search'
  ...
  ipcMain.handle(
    ...
    async (_e, op: { prompt?: unknown; aspectRatio?: unknown }) => {
      if (!hasGskAuth())
        return {
          error:
            'Trivena Cloud account is not logged in on this machine; ask the user to log in first',
        }
...
/** Standalone window mode: `npm run dev -w @trivoffice/pdf`, pdf path passed via argv */
