...
beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), 'trivoffice-save-dir-'))
})
...
describe('resolveDefaultSaveDir', () => {
  ...
  it('creates and returns the fallback when nothing is configured', () => {
    const fallback = join(root, 'Documents', 'TrivOffice')
...
describe('configuredDefaultSaveDir', () => {
  ...
  it('falls back to <Documents>/TrivOffice without a setting', () => {
    const userData = join(root, 'userData')
    const documents = join(root, 'Documents')
    mkdirSync(userData, { recursive: true })
    const app = {
    ...
    expect(configuredDefaultSaveDir(app)).toBe(join(documents, 'TrivOffice'))
    expect(existsSync(join(documents, 'TrivOffice'))).toBe(true)
  })
})
