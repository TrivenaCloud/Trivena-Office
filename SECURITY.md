...
[private vulnerability reporting](https://github.com/TrivenaCloud/Trivena-Office/security/advisories/new)
...
- Every `shell.openExternal` call goes through a single shared gate
  (`@trivoffice/electron-utils` → `safeExternalUrl`) that parses the URL and
