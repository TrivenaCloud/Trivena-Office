 ...
 * userData dir (via TRIVOFFICE_USER_DATA) so runs never touch real settings
...
interface LaunchOptions {
  ...
  userDataDir?: string
  /** UI language override (TRIVOFFICE_LANG); defaults to English for stable assertions */
  lang?: string
...
export async function launchShell(options: LaunchOptions): Promise<LaunchedApp> {
  ...
  const userDataDir = options.userDataDir ?? (await mkdtemp(join(tmpdir(), 'trivoffice-e2e-')))
  ...
  const app = await electron.launch({
    ...
    env: {
      ...hostEnv,
      TRIVOFFICE_USER_DATA: userDataDir,
      TRIVOFFICE_LANG: options.lang ?? 'en',
